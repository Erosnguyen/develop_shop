...
from config.settings import MAX_FILE_SIZE, MEDIA_DIR


# TODO set permission to access media-directory and files
class MediaService:
    def __init__(
        self, parent_directory: str = "media", sub_directory: str | int = None
    ):
        testing = DatabaseManager.get_testing_mode()
        if not testing:
            self.path = Path(
                f"{MEDIA_DIR}/{parent_directory}/{sub_directory}"
                if sub_directory
                else parent_directory
            )
        else:
            self.path = Path(
                f"{MEDIA_DIR}/test/{parent_directory}/{sub_directory}"
                if sub_directory
                else parent_directory
            )
