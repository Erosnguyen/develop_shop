...
class EmailService:
    ...
    def __send_email(cls, subject: str, body: str, to_address: str):
        try:
            ...
            with smtplib.SMTP_SSL(
                cls.config.smtp_server, cls.config.smtp_port
            ) as server:
                # server.set_debuglevel(1)
                server.login(cls.config.smtp_username, cls.config.smtp_password)
                server.sendmail(
                    cls.config.smtp_username, to_address, message.as_string()
                )
                # server.quit()
    ...
    def reset_password_send_verification_email(cls, to_address):
        ...
        body = (
            f"We received a request to reset your {cls.app.app_name} password.\n\n"
    ...
    def change_email_send_verification_email(cls, new_email: str):
        ...
        body = (
            f"We received a request to change the email associated with your {cls.app.app_name} account.\n\n"
