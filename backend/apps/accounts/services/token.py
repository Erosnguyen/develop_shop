...
class TokenService:
    ...
    def create_access_token(self) -> str:
        ...
        to_encode.update(
            {
                "exp": datetime.utcnow()
                + timedelta(self.app_config.access_token_expire_minutes)
            }
        )
    ...
    def update_access_token(self, token: str):
        UserVerification.update(
            UserVerification.filter(UserVerification.user_id == self.user_id)
            .first()
            .id,
    ...
    def create_otp_token(cls):
        totp = TOTP(
            cls.app_config.otp_secret_key, interval=cls.app_config.otp_expire_seconds
        )
        return totp.now()
    ...
    def get_new_email(self):
        _change: UserVerification = UserVerification.filter(
            UserVerification.user_id == self.user_id
        ).first()
    ...
    def request_is_change_email(self, new_email: str):
        _change = (
            UserVerification.filter(UserVerification.user_id == self.user_id).first().id
        )
        UserVerification.update(
            _change, new_email=new_email, request_type="change-email"
        )
