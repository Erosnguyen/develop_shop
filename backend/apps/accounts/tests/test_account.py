...
class TestRegisterAccount(AccountTestBase):
    ...
    def test_successful_register_with_email(self):
        ...
        assert (
            expected["message"]
            == "Please check your email for an OTP code to confirm your email address."
        )
        ...
        assert (
            PasswordManager.verify_password(payload["password"], expected_user.password)
            is True
        )
    ...
    def test_successful_verify_registration(self):
        ...
        assert (
            PasswordManager.verify_password(
                FakeAccount.password, expected_user.password
            )
            is True
        )
        ...
        change_request: UserVerification = UserVerification.filter(
            UserVerification.user_id == expected_user.id
        ).first()
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"email": "valid@test.com", "password": "PASSWORD_t1"},
            {
                ...
                "password": "PASSWORD_t1",
                "password_confirm": "PASSWORD_t1",
            },
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {
                "password": "Pass1_Pass1_Pass1_Pass1_1",
...
class TestLoginAccount(AccountTestBase):
    def test_successful_login(self):
        ...
        user, _ = FakeAccount.verified_registration()
        payload = {"username": user.email, "password": FakeAccount.password}
    ...
    def test_login_with_incorrect_password(self):
        ...
        # --- register and verify a user ---
        user, _ = FakeAccount.verified_registration()
        payload = {"username": user.email, "password": FakeAccount.password + "t"}
    ...
    def test_login_with_inactive_account(self):
        ...
        payload = {"username": email, "password": FakeAccount.password}
    ...
    def test_login_after_password_change(self):
        ...
        old_password = {"username": user.email, "password": passwords["old_password"]}
        response = self.client.post(self.login_endpoint, data=old_password)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

        # --- login with new password ---
        new_password = {"username": user.email, "password": passwords["new_password"]}
    ...
    def test_login_after_password_reset(self):
        ...
        AccountService.verify_reset_password(
            email=user.email, password=new_password, otp=TokenService.create_otp_token()
        )

        # --- login with old password ---
        old_password = {"username": user.email, "password": FakeUser.password}
        response = self.client.post(self.login_endpoint, data=old_password)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

        # --- login with new password ---
        new_password = {"username": user.email, "password": new_password}
        response = self.client.post(self.login_endpoint, data=new_password)
        assert response.status_code == status.HTTP_200_OK
...
class TestResetPassword(AccountTestBase):
    def test_reset_password_with_email(self):
        ...
        assert (
            expected["message"]
            == "Please check your email for an OTP code to confirm the password reset request."
        )
    ...
    def test_verify_reset_password_with_email(self):
        ...
        expected_access_token = (
            UserVerification.filter(UserVerification.user_id == expected_user.id)
            .first()
            .active_access_token
        )
...
class TestResendOTP(AccountTestBase):
    def test_stop_resend_on_register(self):
        ...
        payload = {"request_type": "register", "email": email}
    ...
    def test_stop_resend_on_change_email(self):
        ...
        # --- request for resend an OTP ---
        payload = {"request_type": "change-email", "email": user.email}
        response = self.client.post(self.otp_endpoint, json=payload)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "OTP not expired. Resend available in " in response.json().get("detail")
    ...
    def test_stop_resend_on_reset_password(self):
        ...
        # --- request for resend an OTP ---
        payload = {"request_type": "reset-password", "email": user.email}
        response = self.client.post(self.otp_endpoint, json=payload)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "OTP not expired. Resend available in " in response.json().get("detail")
    ...
    def test_resend_on_change_email_invalid_request_type(self, invalid_request_type):
        ...
        # --- request for resend an OTP ---
        payload = {"request_type": invalid_request_type, "email": user.email}
        response = self.client.post(self.otp_endpoint, json=payload)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
