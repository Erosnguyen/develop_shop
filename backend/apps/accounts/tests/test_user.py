...
class TestRetrieveUser(UserTestBase):
    ...
    def test_retrieve_single_user(self):
        ...
        response = self.client.get(
            f"{self.accounts_endpoint}{user.id}", headers=headers
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_retrieve_single_user_403(self):
        ...
        # --- request to fetch user data from token ---
        response = self.client.get(
            f"{self.accounts_endpoint}{user_2.id}", headers=headers
        )
...
class TestUpdateUser(UserTestBase):
    def test_update_current_user(self):
        ...
        response = self.client.put(
            self.current_user_endpoint, headers=headers, json=payload
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestChanges(UserTestBase):
    ...
    def test_change_password(self):
        ...
        response = self.client.patch(
            self.change_password_endpoint, headers=header, json=payload
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_change_email(self):
        ...
        response = self.client.post(
            self.change_email_endpoint, headers=header, json=payload
        )
        assert response.status_code == status.HTTP_200_OK
        ...
        assert expected["message"] == (
            f'Please check your email "{payload["new_email"]}" for an OTP code to'
    ...
    def test_verify_email_change_request(self):
        ...
        response = self.client.patch(
            self.verify_change_email_endpoint, headers=header, json=payload
        )
        assert response.status_code == status.HTTP_200_OK
