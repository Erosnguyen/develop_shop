...
class TestCreateProduct(ProductTestBase):
    ...
    def test_create_product(self):
        ...
        * every time we create product, the media should be None, because the Media after creating a product will be
        ...
        response = self.client.post(
            self.product_endpoint, json=payload, headers=self.admin_authorization
        )
    ...
    def test_create_product_required(self):
        ...
        # --- request ---
        payload = {"product_name": "Test Product"}
        response = self.client.post(
            self.product_endpoint, json=payload, headers=self.admin_authorization
        )
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_payload_is_empty(self):
        ...
        response = self.client.post(
            self.product_endpoint, json={}, headers=self.admin_authorization
        )
    ...
    @pytest.mark.parametrize(
        "status_value", ["", None, "blob", 1, False, "active", "archived", "draft"]
    )
    def test_payload_invalid_status(self, status_value):
        ...
        payload = {"product_name": "Test Product", "status": status_value}
    ...
    def test_payload_invalid_options(self, options_value):
        ...
        payload = {"product_name": "Test Product", "options": options_value}
    ...
    def test_payload_invalid_price(self, price_value):
        ...
        payload = {"product_name": "Test Product", "price": price_value}
    ...
    def test_payload_invalid_stock(self, stock_value):
        ...
        payload = {"product_name": "Test Product", "stock": stock_value}
...
class TestRetrieveProduct(ProductTestBase):
    ...
    def test_retrieve_product_with_options_media(self):
        ...
        # --- create a product ---
        payload, product = asyncio.run(
            FakeProduct.populate_product_with_options_media()
        )
...
class TestUpdateProduct(ProductTestBase):
    ...
    def test_update_product(self, update_payload):
        ...
        response = self.client.put(
            f"{self.product_endpoint}{product.id}",
            json=update_payload,
        ...
        assert expected["created_at"] == self.convert_datetime_to_string(
            product.created_at
        )
...
class TestDestroyProduct(ProductTestBase):
    ...
    def test_delete_product(self):
        ...
        response = self.client.delete(
            f"{self.product_endpoint}{product.id}", headers=self.admin_authorization
        )
        assert response.status_code == status.HTTP_204_NO_CONTENT
