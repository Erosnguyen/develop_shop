...
from apps.products.faker.data import FakeMedia, FakeProduct
...
class TestCreateProductMedia(ProductMediaTestBase):
    ...
    def test_create_product_media(self):
        ...
        media_payload = {"alt": "Test Alt Text"}

        # --- request ---
        response = self.client.post(
            f"{self.product_endpoint}{product.id}/media/",
            data=media_payload,
            files=files,
...
class TestUpdateProductMedia(ProductMediaTestBase):
    ...
    async def test_update_media(self):
        ...
        # --- get a media ---
        media = ProductService.retrieve_media_list(product.id)[0]
        update_payload = {"media_id": media["media_id"], "alt": "updated alt text"}
...
class TestDestroyProductMedia(ProductMediaTestBase):
    ...
    async def test_delete_media_file(self):
        ...
        response = self.client.delete(
            f"{self.product_media_endpoint}{media_id}", headers=self.admin_authorization
        )
        assert response.status_code == status.HTTP_204_NO_CONTENT
