...
class FakeProduct:
    ...
    def populate_product(cls) -> tuple[dict[str, str | int], Product]:
        ...
        return product_data.copy(), ProductService.create_product(
            product_data, get_obj=True
        )

    @classmethod
    def populate_product_with_options(
        cls, get_product_obj=True
    ) -> tuple[dict[str, str | int], Product | dict]:
        """
        ...
        return product_data.copy(), ProductService.create_product(
            product_data, get_obj=get_product_obj
        )
    ...
    async def populate_product_with_media(cls):
        ...
        # --- get demo images ---
        upload = FakeMedia.populate_images_for_product(
            upload_file=True, product_id=product.id
        )
